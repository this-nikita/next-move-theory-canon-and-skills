...
- **X is not a local-optimum target; X is a global-optimum scouting surface.** Don't try to "convert X to A" by tweaking the current funnel — the Core Jobs mismatch is structural, not messaging. The right move is *map X's Jobs* and ask *"would building a product for these Jobs produce its own A/B?"*
