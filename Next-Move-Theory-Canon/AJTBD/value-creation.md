...
This file is the detailed canon page on **value** in AJTBD: what value is, how it is detected inside the customer's brain, the structure of a value-creation move, the catalog of mechanics that produce value, and how to apply them. It expands the short version in [ajtbd-key-theses.md §6](ajtbd-key-theses.md) and connects directly to [behaviour-change.md](behaviour-change.md) (every value-delivery event is mechanistically a behavior-change event) and to [job-graph.md](job-graph.md) (value mechanics are moves *in* the Job Graph).

> This page is plain-English methodology. The underlying neuroscience (allostasis, neural common currency, reward prediction error, loss aversion, dopamine as a learning signal, etc.) lives in [scientific-foundations.md](scientific-foundations.md); when this page references a brain-based mechanism, it links there rather than re-explaining the science inline.
...
**Why this shape — multiplicative on `Probability × Outcome`, subtractive on Cost (not pure `Benefit − Cost`, not `Benefit / Cost`) — and the two anchoring studies live in [`scientific-foundations.md §2`](scientific-foundations.md).**
...
The underlying neuroscience is unpacked in [scientific-foundations.md §11](scientific-foundations.md). For this file: the brain is a predict-and-compare engine; *behaviour change is driven by the delta — value itself is the absolute energy-efficiency the Solution delivers (§2).*

## 5. Positive and Negative Prediction Errors — the brain's re-learning loop

The predict-and-compare engine of §4 produces a signed delta. The sign matters: each sign triggers a *different teaching signal* in the brain, and behavior reshapes accordingly. This section stays at the logic level — for the underlying neurobiology (dopamine as the teaching signal, the goal-directed vs. habit shift inside the striatum) see [scientific-foundations.md](scientific-foundations.md) and [behaviour-change.md §7](behaviour-change.md).
...
The mechanism is detailed in [behaviour-change.md §7](behaviour-change.md).
...
*(Switching cost is a **composite** of effort, cognitive load, and negative emotions paid to leave the current Solution — see [behaviour-change.md §9](behaviour-change.md) for treatment as a force blocking behavior change.)*
...
Canonical treatment in [ajtbd-key-theses.md §5](ajtbd-key-theses.md); this section expands the value-creation-specific implications.
...
**Identifying the priority criterion (or criteria) of your focus segment is what tells you which mechanic from [`value-creation-mechanics.md`](value-creation-mechanics.md) to lead with.** Segmentation by Job-Graph similarity (see [segmentation.md](segmentation.md)) routes through *priority orders over criteria*, not through demographics. Two segments with identical demographics can have inverted criteria orders — same product strategy wins one, loses the other.
...
Across the whole 100+ catalog, two mechanics dominate the rest in raw leverage whenever they are applicable. Both have the same structural property: **they don't add features, they remove Jobs.** They are the AJTBD-side projection of the cross-pillar subtraction operator ([Next-Move-Theory/subtraction.md §1](../Next-Move-Theory/subtraction.md)).

### 14.1 Move up a level

Climb one level in the Job Graph: turn a Big Job that sits above your current Core Jobs into your new Core Jobs, and in the process kill many of the lower-level Jobs that the customer previously had to perform. (See [ajtbd-key-theses.md §23](ajtbd-key-theses.md).)
...
The pattern: once you perform your Core Jobs to their criteria, the next unit of value is rarely inside your current Core Jobs. It is across the boundary — in the Previous Job (lead-gen, capture earlier in the chain), the Next Job (cross-sell, retention), the sibling Small Job (a Job at the same level as yours, serving the same Big Job, performed by the customer themselves or by another Solution — performer-agnostic; see [job-graph.md §2](job-graph.md)), or one level up (move-up-a-level, §14). **Or — equally valid — go create value for the *Core Jobs of a different segment* entirely.** Once your current segment's Core Jobs land at criteria, scaling to an adjacent segment's Core Jobs is often a larger value pool than further expanding within the current segment's Job Graph.

## 18. The mechanics catalog

**The canonical catalog of value-creation mechanics — with detailed descriptions, example clusters, Russian translations, and cross-refs to deeper canon — lives in [`value-creation-mechanics.md`](value-creation-mechanics.md).** That file is the entry-level slice every product person should know cold; the broader 100+ Next Move Theory catalog (including go-to-market, retention, pricing, scaling, exit-from-direct-competition) lives in [`Next-Move-Theory/mechanics-catalog.md`](../Next-Move-Theory/mechanics-catalog.md) — internal-only.

> **Each mechanic, without exception, grounds out into one of three things** — a coarser grouping of the six foundations in [ajtbd-key-theses.md §22](ajtbd-key-theses.md):
...
Value that arrives *in the future* is systematically underweighted by the brain — the temporal-discounting / hyperbolic-discounting literature (see [scientific-foundations.md §14](scientific-foundations.md)). It shapes how the customer evaluates products and creates a specific kind of communication problem.
...
- **Communication strategy.** Rewrite the deferred benefit as a present-tense experience the brain credits immediately: peace of mind tonight; the auditor signing the same day; three weeks of release-blocker review saved on the next launch. (Detailed examples and copy patterns in [communication.md](communication.md).)
- **Operational connection:** this is the *Remove negative emotions* mechanic ([value-creation-mechanics.md](value-creation-mechanics.md)) applied to a deferred value. Sell *the elimination of present-tense anxiety* about a future loss, not the future loss itself.
...
**Two independent traditions converge on this limit.** AJTBD arrives at the invisible product from Job-and-value logic. Genrikh Altshuller's TRIZ arrived at the same limit case from engineering theory in the 1950s — the *Ideal Final Result*, where the function is performed *without a system to perform it* (see [scientific-foundations.md §21](scientific-foundations.md)). The cross-pillar treatment — invisible product as the asymptote of the meta-operator across all four NMT pillars — lives in [Next-Move-Theory/subtraction.md §3](../Next-Move-Theory/subtraction.md).

---

For deeper unpacks of specific topics referenced above:

- [job-graph.md](job-graph.md) — the Job Graph, levels, moves
- [critical-chain.md](critical-chain.md) — critical chains, breaks, cycle removal
- [behaviour-change.md](behaviour-change.md) — full mechanism of how Aha Moment/Problem rewires customer behavior
- [consideration-activators.md](consideration-activators.md) — the Consideration Activators construct that makes Class-2 (unfamiliar Core Job) switches possible
- [scientific-foundations.md](scientific-foundations.md) — AJTBD's key hypothesis of value (§2), neural common currency (§3), purchase-decision neuroscience (§4), the AJTBD need taxonomy (§5), status as rank-position motivation (§6), identity-based motivation (§7), Jobs as need-serving goal representations (§8), habit physiology (§15), Immunity to Change (§16), variable-ratio reward loops (§18)
- [communication.md](communication.md) — how to convey delivered value, formulas, landing-page structure
- [Next-Move-Theory/subtraction.md](../Next-Move-Theory/subtraction.md) — subtraction as the meta-operator across all four NMT pillars; the four-way ROI asymmetry that makes removing beat adding; the invisible-product asymptote; the structural team bias against subtraction
- [Algorithms/create-value.md](../Algorithms/create-value.md) — step-by-step algorithm for applying these mechanics to a real product situation
