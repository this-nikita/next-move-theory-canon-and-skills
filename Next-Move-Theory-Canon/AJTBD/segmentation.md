...
For product strategy, value creation, positioning, and differentiation, **the first causal segmentation criterion is Job Graph similarity around similar Core Jobs with similar success criteria.** A segment is a set of customers who perform similar **Core Jobs** with similar **success criteria** — that pair is the segmentation root. The **Big Jobs** above carry motivation and refine the segment, but are not a co-equal segmentation criterion (same Big Job ≠ same segment); the Core Job is what the product competes to perform, and the criteria make the Job concrete enough to build, sell, and price.
...
- **Before applying a filter, write what it removes.** If the removed slice might contain people who paid for the relevant Core Jobs or Big Jobs, the filter is too early.
...
- **Can we earn target per-unit margin while competing for these Core Jobs of these people?** Minimum evidence: Job budget; willingness-to-pay; cost-to-serve; acquisition cost.
- **Can we create or capture demand for these Core Jobs of these people?** Minimum evidence: Consideration Activators; channel proxies; sales motion.
...
- **Treat focus as segment-level subtraction.** Choosing a focus segment subtracts non-focus Core Jobs from the investment zone until the focus segment is won — the AJTBD form of [Subtraction](../Next-Move-Theory/subtraction.md).
...
- **Local-optimum blindness.** The current A/B segment is improved while a larger adjacent segment, a move up a level to a Big Job, or a business-model shift remains unexplored.

**Operational consequences:**

- **Before fixing a metric, ask whether the metric belongs to the focus segment.** Churn, conversion, support load, and activation are actionable only after segment membership is known.
- **Before building a feature, ask which focus-segment Core Job it improves.** If the answer is *"several segments"* or *"all users,"* the work needs decomposition.
