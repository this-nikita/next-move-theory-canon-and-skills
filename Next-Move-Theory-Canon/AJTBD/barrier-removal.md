...
**A Barrier is an objective condition that prevents the customer from performing the new Job Graph/Critical Chain.** The condition can live before the chain starts, inside a chain node, at a hand-off, inside a cycle, or at the point where the chain has to prove the Big Jobs landed. If a missing prerequisite sits before the first visible action, treat it as node zero of the chain.

**The customer usually knows the Barrier exists and uses it as the reason not to enter the new Job Graph with our Solution.** Unlike fears (predictions about a break that may or may not happen) and Consideration Activators gaps (things the customer doesn't know to look for), a Barrier is typically a fact the customer can name the moment we ask — *"my state isn't supported," "you don't have SSO yet," "you don't deliver to my ZIP code," "the import failed on my data."* The customer hits the fact, recognizes it as a blocker, and routes around our Solution — most of them silently, without ever telling us.
...
**A Barrier is segment-specific.** The same product can be executable for one segment and blocked for another. Stripe's API worked for YC-stage startups with engineers; local merchants needed no-code checkout. Zoom worked for corporate meetings; K-12 schools needed FERPA, waiting rooms, locked meetings, and classroom controls. The product did not become bad — the segment's Critical Chain changed.

## 2. Barrier Removal creates the possibility to switch Job Graphs

**Value Creation creates the reason to switch Job Graphs; Barrier Removal creates the possibility to switch Job Graphs.** The customer does not merely switch a vendor label. They stop performing the old Core Jobs / Small Jobs / Micro Jobs in one graph and begin performing a new graph with our Core Jobs inside their Big Jobs. The Solution label changes because the graph changed.

**Barrier Removal is the work of changing reality so the new Job Graph can actually be performed — not persuasion.** It changes prerequisites, hand-offs, integrations, permissions, payment paths, logistics, legal paths, data formats, geography, role coverage, and broken chain links. If a migration path does not exist, a testimonial is not the fix; if a state is unsupported, a guarantee is not the fix; communication can reduce a fear only after the feared break is not true.

**Examples:** Stripe Checkout made *"charge customers online"* executable for non-engineering merchants; Notion Enterprise made *"adopt Notion org-wide"* executable inside IT/security review; Belong made *"rent out my duplex"* executable for out-of-state owners; TurboTax K-1 support made *"file my return correctly"* executable for LLC owners.
...
- **Cycle / return-for-rework.** The customer reaches a step, gets sent back, and pays the same Job again as a Tax Job: mortgage document cycles, prior-authorization denials, insurance-claim resubmissions, tax-prep missing-form loops.
...
## 7. Barrier Removal is researched through motivated customers who hit the Barrier — whether mid-chain or before entering

**The first source: motivated customers who tried and could not complete the chain.** Existing customers already passed the hidden blocker; non-motivated prospects are noise. The signal is the customer who wanted the Big Jobs, accepted the value, entered the transition, and got stuck.

**The second source: motivated customers who saw the Barrier up front and chose not to try.** This pool is invisible to standard funnel analytics — they never entered. Find them by recruiting on motivation (right Big Jobs, right segment) without filtering on whether they used the product. Interview question: *"Was there anything you saw or heard about {studied Solution} that made you not even try it?"* The pre-entry opt-out cohort is harder to source than the failed-attempt cohort, but it surfaces the largest single class of Barrier — the one the customer can name before walking the chain.

**The research object is the exact Job where the chain became non-executable.** Not *"they didn't convert"*; *"payment method failed."* Not *"enterprise didn't buy"*; *"security review blocked because user-provisioning integration was missing."* Not *"landlords don't trust us"*; *"out-of-state access, repairs, and lease handoff had no runnable path."*
